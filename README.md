...
`dc2` supports an optional YAML test profile for delay injection across
`RunInstances`/`StartInstances`/`StopInstances`/`TerminateInstances`, plus
spot reclaim overrides for `RunInstances`-based flows (including Auto Scaling
warm pool launches).
...
| Area | Status | Notes |
| --- | --- | --- |
| EC2 Instances | Partial | Lifecycle APIs plus IMDSv2 instance-id/user-data/tag metadata support, including `RunInstances` launch-template references with request-field overrides (`ImageId`/`InstanceType`/`UserData`/block device mappings). |
