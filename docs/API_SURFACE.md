...
| Auto Scaling Group | `CreateAutoScalingGroup` | Supported | Requires launch template image and instance type; applies launch template `UserData` and `BlockDeviceMapping[].Ebs` to launched instances; accepts `Tags.member.N` entries with ASG resource tags. `PropagateAtLaunch=true` tags are now propagated to ASG-launched instances (including replacement and warm-pool launches). |
