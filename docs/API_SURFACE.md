...
| --- | --- | --- | --- |
| Instance | `RunInstances` | Partial | Launches container-backed instances, including `UserData` storage for IMDS and container-derived private/public IP metadata. |
| Instance | `DescribeInstances` | Partial | Supports IDs/tag filters (`tag:*`, `tag-key`) and returns private/public IP metadata from container networking. |
